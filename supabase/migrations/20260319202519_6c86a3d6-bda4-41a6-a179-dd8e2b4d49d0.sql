...
-- REVERT: Remove is_super_admin() bypass from all RESTRICTIVE policies
-- This restores org isolation for dual-role users on the admin dashboard
...
USING (NOT is_org_member() OR organization_id = get_user_organization_id());
...
AS RESTRICTIVE FOR ALL TO authenticated
USING (NOT is_org_member() OR organization_id = get_user_organization_id() OR organization_id IS NULL);
...
-- Create SECURITY DEFINER function for super admin to get org client counts (bypasses RLS)
CREATE OR REPLACE FUNCTION public.get_org_client_counts()
RETURNS TABLE(organization_id uuid, client_count bigint)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $$
  SELECT c.organization_id, COUNT(*)::bigint AS client_count
