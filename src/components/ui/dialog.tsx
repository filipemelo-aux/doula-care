...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        "fixed left-[50%] top-[50%] z-[100] grid w-[calc(100%-2rem)] max-w-lg translate-x-[-50%] translate-y-[-50%] border-0 bg-background shadow-[0_10px_30px_rgba(0,0,0,0.08)] rounded-[20px] duration-200 overflow-visible max-h-[85vh] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]",
        className,
      )}
      {...props}
    >
      <div className="overflow-y-auto overscroll-contain p-5 pb-7 gap-3 grid" style={{ WebkitOverflowScrolling: 'touch', maxHeight: '85vh' }}>
        {children}
      </div>
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full opacity-40 ring-offset-background transition-opacity hover:opacity-70 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
